 ...
 * same choices — which columns, which format, which orientation — instead
 * of each one wiring its own buttons. `headers`/`rows` are the exact arrays
 * a page would hand `downloadCSV`; this only ever exports a subset of them.
 */
export default function ExportMenu({ filename, title, subtitle, headers, rows, statusColumnIndex, disabled }) {
  ...
  const run = async () => {
    ...
    const subHeaders = order.map(i => headers[i])
    const subRows = rows.map(r => order.map(i => r[i]))
    ...
    try {
      await exportPDF({
        filename: `${filename}.pdf`, title, subtitle,
        headers: subHeaders, rows: subRows, statusColumnIndex: subStatusIndex, orientation,
      })
    } finally {
      setBusy(false)
      setOpen(false)
